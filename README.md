...
- 큐레이팅 API 구현
  - 큐레이팅 생성
    - 중복 방지 - 사용자가 같은 스타일에 한번만 큐레이팅 등록이 되게 제한
    - 스타일 존재 여부 확인 - 스타일 게시글(styleId)이 존재해야 생성 가능
    - 비속어/금칙어(씨x, 존나) 내용 제한 - 내용 입력 칸에 비속어/금칙어가 있으면 등록 못하게 제한
  - 큐레이팅 목록 보기
    - 스타일 존재 여부 확인 - 스타일 게시글(styleId)이 존재해야 조회 가능
    - 닉네임, 한줄 큐레이팅 내용을 기준으로 검색이 가능하게 완료
    - 큐레이팅에 남겨진 답글도 같이 조회되게 완료
  - 큐레이팅 수정
    - 스타일 존재 여부 확인 - 스타일 게시글(styleId)이 존재해야 수정 가능
    - 비밀번호 인증 - 작성자만 수정 가능(비밀번호가 일치해야 수정 가능)
  - 큐레이팅 삭제
    - 스타일 존재 여부 확인 - 스타일 게시글(styleId)이 존재해야 삭제 가능
    - 비밀번호 인증 - 작성자만 삭제 가능(비밀번호가 일치해야 삭제 가능)
- 랭킹 API 구현
  - 전체, 트렌디, 개성, 실용성, 가성비 기준으로 스타일 랭킹 목록을 조회되게 완료
  - 각 스타일의 대표 이미지, 제목, 닉네임, 태그, 스타일 구성, 조회수, 큐레이팅수가 표시됨
  - 페이지네이션이 가능하게 완료
...
- style API 부분 구현
  - 목록 조회, 각 스타일의 표시, 태그, 페이지네이션, 정렬, 검색
  - 1.목록 조회
    - 갤러리에 등록된 스타일 목록을 조회할 수 있게 함.
  - 2.각 스타일 표시
    - 대표 이미지, 제목, 닉네임, 태그, 스타일 구성, 스타일 설명, 조회수 큐레이팅 수가 표시되게 함.
  - 3.태그
    - 갤러리 상단에 인기 태그가 표시되고 태그 클릭 시 그 태그에 해당하는 스타일 목록이 표시되게 함.
  - 4.페이지네이션
    - 한 페이지에서 여러 개의 등록된 스타일을 확인할 수 있게 함.
  - 5.정렬
    - 최신순, 조회순, 큐레이팅순으로 정렬 가능하게 함.
  - 6.검색
    - 닉네임, 제목, 상세, 태그로 검색이 가능하게 함.
