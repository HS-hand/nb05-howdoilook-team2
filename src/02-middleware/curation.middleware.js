...
export class CurationMiddleware {
  ...
  createCurationMiddleware = async (req, res, next) => {
    const createCurationReqDto = new CreateCurationReqValidator({ params: req.params, body: req.body }).validate();
    const createdCuration = await this.#curationService.createCuration(createCurationReqDto);
  ...
  updateCurationMiddleware = async (req, res, next) => {
    const updateCurationReqDto = new UpdateCurationReqValidator({ params: req.params, body: req.body }).validate();
    const updatedCuration = await this.#curationService.updateCuration(updateCurationReqDto);
  ...
  deleteCurationMiddleware = async (req, res, next) => {
    const deleteCurationReqDto = new DeleteCurationReqValidator({ params: req.params, body: req.body }).validate();
    const deletedwCuration = await this.#curationService.deleteCuration(deleteCurationReqDto);
