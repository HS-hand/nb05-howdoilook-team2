...
export class CreateCurationReqValidator extends BaseValidator {
  ...
  validate() {
    ...
    if (!this.isString(content) || this.isEmpty(content)) {
    ...
    if (!this.isInt(trendy) || this.isEmpty(trendy)) {
      throw new Exception(EXCEPTIONS.TRENDY_FORM);
    }

    if (!this.isInt(personality) || this.isEmpty(personality)) {
      throw new Exception(EXCEPTIONS.PERSONALITY_FORM);
    }

    if (!this.isInt(practicality) || this.isEmpty(practicality)) {
      throw new Exception(EXCEPTIONS.PRACTICALITY_FORM);
    }

    if (!this.isInt(costEffectiveness) || this.isEmpty(costEffectiveness)) {
